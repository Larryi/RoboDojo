...
if "self._sidecar_rows = {}" not in text:
    ...
    init = ("        self.preserve_global_task = preserve_global_task\n"
            ...
            "        self._sidecar_rows = {}\n"
            "        self._sidecar_rows_by_global_index = {}\n"
            "        self._manifest_global_indices = None\n"
            "        self._manifest_current_global_index = None\n"
    ...
    new_additional = ("    def _get_additional_data(self, sample, lerobot_sample):\n"
                      ...
                      "            row = self._sidecar_rows_by_global_index.get(self._manifest_current_global_index)\n"
                      ...
                      "            if row is None and episode_index is not None and frame_index is not None:\n"
                      "                row = self._sidecar_rows.get((int(episode_index), int(frame_index)))\n"
                      "            if row is None and episode_index is not None and frame_index is not None:\n"
                      "                # LeRobot v3 exposes frame_index as a global dataset\n"
                      "                # index, while RoboDojo annotations use an episode-local\n"
                      "                # frame. Translate before the sidecar lookup.\n"
                      "                episode_start = int(self.episode_data_index[\"from\"][int(episode_index)])\n"
                      "                local_frame_index = int(frame_index) - episode_start\n"
                      "                row = self._sidecar_rows.get((int(episode_index), local_frame_index))\n"
                      "            if row is None:\n"
                      "                raise RuntimeError(f\"Subgoal manifest row missing for global_index={self._manifest_current_global_index}, episode={episode_index}, frame={frame_index}\")\n"
                      "            if row:\n"
    ...
    methods = ("    def _load_subgoal_manifest(self, path: Path):\n"
               ...
               "            global_index = int(self.episode_data_index[\"from\"][episode]) + frame\n"
               "            self._sidecar_rows_by_global_index[global_index] = row\n"
    ...
    new_getitem = ("    def __getitem__(self, idx):\n"
                   ...
                   "            self._manifest_current_global_index = original_idx\n"
                   "            try:\n"
