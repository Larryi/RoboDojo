...
if [[ -n "${G05_VAST_SECRETS:-}" ]]; then
  ...
  for key in MODELSCOPE_API_TOKEN WANDB_API_KEY SERVERCHAN_SENDKEY HF_DATASET_REPO HF_OUTPUT_REPO \
    G05_GPUS G05_SAVE_INTERVAL_STEPS G05_KEEP_CHECKPOINTS G05_DATASET_SOURCE G05_TRAIN_TASK \
    G05_TRAIN_ARGS G05_RUN_ID G05_MAX_STEPS G05_AUTO_RESUME G05_VIDEO_BACKEND G05_LINEAR_ATTN_BACKEND G05_TORCH_COMPILE G05_BATCH_SIZE \
    G05_LEARNING_RATE G05_LR_MIN_RATIO G05_WARMUP_RATIO G05_CONSTANT_END_RATIO; do
    emit_env "${key}"
  done
fi
